...
pub const ComplexPLLBlock = struct {
    ...
    pub fn init(loop_bandwidth: f32, frequency_min: f32, frequency_max: f32, options: Options) ComplexPLLBlock {
        return .{ .block = Block.init(@This()), .loop_bandwidth = loop_bandwidth, .frequency_min = frequency_min, .frequency_max = frequency_max, .options = options };
    }

    pub fn initialize(self: *ComplexPLLBlock, _: std.mem.Allocator) !void {
        // Translate frequencies
        self.omega_min = 2 * std.math.pi * (self.frequency_min / self.block.getRate(f32));
        self.omega_max = 2 * std.math.pi * (self.frequency_max / self.block.getRate(f32));
