...
pub fn ApplicationSink(comptime T: type) type {
    return struct {
        ...
        pub fn wait(self: *Self, min_count: usize, timeout_ns: ?u64) error{ EndOfFile, Timeout }!void {
            return self.sample_mux.vtable.waitInputAvailable(self.sample_mux.ptr, 0, min_count * @sizeOf(T), timeout_ns);
        }

        pub fn available(self: *Self) error{EndOfFile}!usize {
...
test "ApplicationSink wait, available, get, update, read, eof" {
    // Create ring buffers
    ...
    // Available should return EOF
    try std.testing.expectError(error.EndOfFile, application_sink.available());

    // Wait should return EOF
    try std.testing.expectError(error.EndOfFile, application_sink.wait(1, std.time.ns_per_ms));
}
