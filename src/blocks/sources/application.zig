...
pub fn ApplicationSource(comptime T: type) type {
    return struct {
        ...
        pub fn wait(self: *Self, min_count: usize, timeout_ns: ?u64) error{Timeout}!void {
            return self.sample_mux.vtable.waitOutputAvailable(self.sample_mux.ptr, 0, min_count * @sizeOf(T), timeout_ns);
        }

        pub fn available(self: *Self) usize {
            return self.sample_mux.vtable.getOutputAvailable(self.sample_mux.ptr, 0) / @sizeOf(T);
        }
        ...
        pub fn setEOF(self: *Self) void {
            return self.sample_mux.vtable.setEOF(self.sample_mux.ptr);
        }
    };
}
...
test "ApplicationSource available, get, update, write, setEOF" {
    // Create ring buffers
    ...
    application_source.setEOF();

    // Reader should have two samples
    try std.testing.expectEqual(2 * @sizeOf(u32), try output_reader.getAvailable());
    output_reader.update(2 * @sizeOf(u32));

    // Reader should have EOF
